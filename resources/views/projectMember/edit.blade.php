<x-app-layout>
    <div class="p-6 bg-surface dark:bg-dark-surface rounded-lg shadow-soft transition-colors duration-300 max-w-7xl w-auto mx-auto">
        ...
        <form method="POST" action="{{ route('project_members.update', $projmember->id) }}">
            @csrf
            @method('PUT')
                ...
                <select id="project_id" name="project_id"
                    ...
                    @foreach ($projects as $item)
                        <option value="{{ $item->id }}"
                            {{ (old('project_id', $projmember->project_id) == $item->id) ? 'selected' : '' }}>
                            {{ $item->judul }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select id="user_id" name="user_id"
                    ...
                    @foreach ($users as $item)
                        <option value="{{ $item->id }}"
                            {{ (old('user_id', $projmember->user_id) == $item->id) ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <select id="role" name="role"
                    ...
                    <option value="ketua" {{ old('role', $projmember->role) == 'ketua' ? 'selected' : '' }}>Ketua</option>
                    <option value="anggota" {{ old('role', $projmember->role) == 'anggota' ? 'selected' : '' }}>Anggota</option>
                </select>

...
>
        ...
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="flex items-center space-x-10">
                <a href="{{ route('dashboard') }}">
                    <x-application-logo
                        class="block h-9 w-auto fill-current text-primary hover:opacity-90 transition"
                    />
                </a>
                ...
                <div class="hidden sm:flex space-x-6">
                    ...
                    @elseif ($role === 'dosen')
                        ...
                        <x-nav-link
                            ...
                            :active="request()->routeIs('matakuliahs.index')"
                        >
                            {{ __('Mata Kuliah') }}
                        </x-nav-link>
                    @else
                        ...
                        <x-nav-link
                            ...
                            :active="request()->routeIs('prodis.index')"
                        >
                            {{ __('Program Studi') }}
                        </x-nav-link>
            ...
            <div class="hidden sm:flex items-center space-x-4">
                ...
                <button
                    ...
                    class="flex items-center gap-2 px-3 py-1.5 rounded-full text-sm text-text-secondary dark:text-dark-text-secondary bg-border dark:bg-dark-border hover:bg-primary-hover dark:hover:bg-dark-primary-hover hover:text-white transition"
                >
                    <svg
                        x-show="!darkMode"
                        class="h-4 w-4"
                ...
                >
                    <button
                        @click="open = !open"
                        class="flex items-center px-4 py-2 text-sm font-medium text-text-secondary dark:text-dark-text-secondary bg-surface dark:bg-dark-surface hover:text-primary rounded-xl transition"
                    >
                    ...
                    <div
                        x-show="open"
                        x-transition
                        class="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-surface dark:bg-dark-surface border border-border dark:border-dark-border space-y-1 p-1 z-50"
                    >
    ...
    >
        ...
        @if ($role === 'mahasiswa')
            ...
            {{--
                <x-responsive-nav-link
                :href="route('project_members.index')"
                :active="request()->routeIs('project_members.index')"
                >
                {{ __('Proyek Member') }}
                </x-responsive-nav-link>
            --}}
        ...
        <button
            @click="darkMode = !darkMode"
            class="flex items-center gap-2 px-3 py-1.5 rounded-full text-sm text-text-secondary dark:text-dark-text-secondary bg-border dark:bg-dark-border hover:bg-primary-hover dark:hover:bg-dark-primary-hover hover:text-white dark:hover:text-white transition"
        >
            <svg
                x-show="!darkMode"
                class="h-4 w-4"
        ...
        <div class="border-t border-border dark:border-dark-border pt-3">
            <div
                class="text-base font-semibold text-text-primary dark:text-dark-text-primary"
            >
                {{ Auth::user()->name }}
            </div>
            <div

    ...
    <div class="p-6 bg-surface dark:bg-dark-surface rounded-lg shadow-soft transition-colors duration-300 max-w-7xl w-auto mx-auto">
        ...
        <form method="POST" action="{{ route('projects.update', $user->id) }}">
            ...
            <div class="mb-4">
                ...
                <x-text-input id="kd_prodi" name="kd_prodi" type="kd_prodi" :value="old('kd_prodi')" required />
                <x-input-error :messages="$errors->get('kd_prodi')" class="mt-1" />
            </div>
            
            <div class="mb-4">
                <x-input-label for="kd_matakuliah" value="Kd_Matakuliah" />
                <x-text-input id="kd_matakuliah" name="kd_matakuliah" type="kd_matakuliah" :value="old('kd_matakuliah')" required />

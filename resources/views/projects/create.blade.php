    ...
    <div class="p-6 bg-surface dark:bg-dark-surface rounded-lg shadow-soft transition-colors duration-300 max-w-7xl w-auto mx-auto">
        ...
        <form method="POST" action="{{ route('projects.store') }}">
            ...
            <div class="mb-4">
                ...
                <x-text-input id="deskripsi" name="deskripsi" type="text" :value="old('deskripsi')" required />
            ...
            <div class="mb-4">
                ...
                <select id="kd_prodi" name="kd_prodi"
                    ...
                    @foreach ($prodis as $item)
                        <option value="{{ $item->kd_prodi }}" {{ old('kd_prodi') == $item->kd_prodi ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <select id="kd_matakuliah" name="kd_matakuliah"
                    ...
                    @foreach ($matakuliah as $item)
                        <option value="{{ $item->kd_matakuliah }}" {{ old('kd_matakuliah') == $item->kd_matakuliah ? 'selected' : '' }}>
           ...
           {{-- NIM Mahasiswa --}}
            <div class="mb-4">
                ...
                {{-- Visible input (readonly for UI) --}}
                <x-text-input id="mahasiswa_nim_display" name="mahasiswa_nim_display" type="text"
                            class="w-full" :value="$userNIM" disabled />

                {{-- Hidden input for submission --}}
                <input type="hidden" name="mahasiswa_nim" value="{{ $userNIM }}">
